...
public class TileScriptableObject : ScriptableObject
{
    ...
    public Task ChangeTiles(PuzzleBoard puzzleBoard, int row, int col, float delayInterval)
    {
        ...
        for (var index = 0; index < flipPattern.Length; index++)
        {
            ...
            if (!puzzleBoard.CanChangeTileColor(targetRow, targetCol))
            {
                continue;
            }

            tasks.Add(puzzleBoard.ChangeTileColor(targetRow, targetCol, delay));
            delay += delayInterval;
        }
